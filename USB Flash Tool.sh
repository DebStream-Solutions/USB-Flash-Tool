...
if [ "$EUID" -ne 0 ]; then
    ...
    if [[ -z "$USB_DEVICE" ]]; then
        ...
        # Check if the chosen disk is in the list of USB disks
        if [[ "$USB_DISKS" == *"$CHOSEN_DISK"* ]]; then
            USB_DEVICE=$CHOSEN_DISK
        else
            echo "Invalid choice. Please run the script again and choose a valid disk."
        fi
    fi
    ...
    echo "Burn | $IMAGE_PATH | >> | /dev/$DEVICE_NAME - $DEVICE_SIZE |"
